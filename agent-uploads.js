/* ── Reading any file people attach to an agent chat ────────────────
...
/* ── Reading uploaded files ─────────────────────────────────────────
   Everything an agent reads for you (a syllabus, an assignment sheet,
   ...
   the picker on a computer, iPad, or phone, so you couldn't even
   select the Word document you had. Anything can be picked now, and a
...
async function readOneUpload(file, { pdfPages = 6, textOnly = false } = {}) {
  ...
  if (kind === 'media') throw new Error(`${name} is audio or video, which can’t be read here. Upload notes, slides, or a transcript instead.`);
  ...
  throw new Error(`Can’t read ${name}. Save it as a PDF, then upload that.`);
}
...
function unreadableByName(name, ext) {
  ...
  if (['mht', 'mhtml', 'webarchive'].includes(ext)) return `${name} is a saved web page, which can’t be read here. Open the page, print it to a PDF, then upload that.`;
  if (['epub', 'mobi', 'azw', 'azw3'].includes(ext)) return `E-books like ${name} can’t be read here. Copy the section you need and paste it in instead.`;
  if (['zip', 'rar', '7z'].includes(ext)) return `${name} is a compressed folder. Unzip it, then upload the files inside.`;
  if (/^(mp3|m4a|wav|aac|flac|ogg|oga|opus|mp4|m4v|mov|avi|mkv|webm|wmv)$/.test(ext)) return `${name} is audio or video, which can’t be read here. Upload notes, slides, or a transcript instead.`;
  return '';
}
function iWorkMessage(name, app) {
  return `${name} is a ${app} file, which can’t be read here yet. Export it as ${app === 'Numbers' ? 'an Excel file' : 'a PDF'} first (File → Export To on a Mac; on an iPad, tap ••• → Export), then upload that.`;
}
...
async function zipUpload(file, ext, { pdfPages, textOnly }) {
  ...
  else throw new Error(ext && ext !== 'zip' ? `Can’t read ${file.name}. Save it as a PDF, then upload that.` : `${file.name} is a compressed folder. Unzip it, then upload the files inside.`);
...
function compoundFileText(cfb, name) {
  ...
  if (cfb.has('Workbook') || cfb.has('Book')) throw new Error(`${name} is an older Excel file (.xls), which can’t be read here yet. Save it as .xlsx or a PDF, then upload that.`);
  throw new Error(`Can’t read ${name}. Save it as a PDF, then upload that.`);
}
...
function docText(cfb, name) {
  const tooOld = () => new Error(`${name} is an older Word file that can’t be read here. Open it in Word, Pages, or Google Docs, save it as .docx or a PDF, then upload that.`);
